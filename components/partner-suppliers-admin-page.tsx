...
const pageCopy: Record<PartnerTypeFilter, {
  singular: string
  plural: string
  ...
  SUPPLIER: {
    singular: "lojista parceiro",
    plural: "lojistas parceiros",
    approvedTitle: "Lojista Parceiro aprovado",
    rejectedTitle: "Lojista Parceiro rejeitado",
    deactivatedTitle: "Lojista Parceiro desativado",
    updatedTitle: "Lojista atualizado",
    emptyTitle: "Nenhum lojista parceiro encontrado",
    description: "Gerencie aprovações e cadastros de lojistas parceiros",
  },
  WELLNESS: {
    singular: "parceiro wellness",
    plural: "parceiros wellness",
    approvedTitle: "Parceiro Wellness aprovado",
    rejectedTitle: "Parceiro Wellness rejeitado",
    deactivatedTitle: "Parceiro Wellness desativado",
    updatedTitle: "Wellness atualizado",
    emptyTitle: "Nenhum parceiro wellness encontrado",
    description: "Gerencie aprovações e cadastros de parceiros wellness",
  },
}

export function SuppliersPageContent({ partnerType = "SUPPLIER" }: SuppliersPageProps) {
  const { suppliers, loading, error, refetch, approve, updateSupplier, reject, grantTrial, cancelTrial, deleteSupplier, updatePointsLimit } =
    useSuppliers()
  const copy = pageCopy[partnerType]
  ...
  const handleApprove = async (id: string) => {
    try {
      ...
      toast({
        ...
        description: `O ${copy.singular} foi aprovado com sucesso.`,
      })
    } catch (error) {
      toast({
        title: "Erro ao aprovar",
        description: `Não foi possível aprovar o ${copy.singular}. Tente novamente.`,
        variant: "destructive",
      })
    }
  }
  ...
  const handleEditConfirm = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `Os dados cadastrais do ${copy.singular} foram salvos com sucesso.`,
      })
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : `Não foi possível atualizar o ${copy.singular}.`,
        variant: "destructive",
      })
      throw error
    }
  ...
  const handleRejectConfirm = async (reason: string) => {
    ...
    try {
      ...
      toast({
        ...
        description: `O ${copy.singular} foi rejeitado com sucesso.`,
      })
    } catch (error) {
      toast({
        title: "Erro ao rejeitar",
        description: `Não foi possível rejeitar o ${copy.singular}. Tente novamente.`,
        variant: "destructive",
      })
      throw error
    }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      await refetch()
      toast({
        title: copy.deactivatedTitle,
        description: `O ${copy.singular} foi desativado com sucesso.`,
      })
    } catch (error) {
      toast({
        title: "Erro ao desativar",
        description: `Não foi possível desativar o ${copy.singular}. Tente novamente.`,
        variant: "destructive",
      })
      throw error
    }
  ...
  const handlePointsLimitConfirm = async (pointsLimit: number) => {
    ...
    try {
      ...
      toast({
        ...
        description: `O limite de pontos do ${copy.singular} foi atualizado com sucesso.`,
      })
    } catch (error) {
      toast({
  ...
  const visibleSuppliers = suppliers.filter((supplier) => (supplier.type ?? "SUPPLIER") === partnerType)

  const filteredSuppliers = visibleSuppliers
    .filter((supplier) => {
      const matchesSearch =
  ...
  const approvedCount = visibleSuppliers.filter((supplier) => supplier.status === "APPROVED").length
  const pendingCount = visibleSuppliers.filter((supplier) => supplier.status === "PENDING").length
  const storesCount = visibleSuppliers.filter((supplier) => supplier.store).length
  const productsCount = visibleSuppliers.reduce((total, supplier) => total + (supplier.store?.products?.length ?? supplier.store?._count?.products ?? 0), 0)
  const eventsCount = visibleSuppliers.reduce((total, supplier) => total + (supplier.store?.events?.length ?? supplier.store?._count?.events ?? 0), 0)

  return (
    <AdminLayout>
      <AdminPageLayout
        title={partnerType === "WELLNESS" ? "Parceiros Wellness" : "Lojista Parceiro"}
      ...
      >
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <Badge variant="outline" className="text-muted-foreground border-border/50 bg-muted/30 font-medium">
            {filteredSuppliers.length} {filteredSuppliers.length === 1 ? copy.singular : copy.plural} encontrado
        ...
        {!loading && filteredSuppliers.length === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold text-card-foreground mb-2">{copy.emptyTitle}</h3>
        ...
        <AdminDialog
          ...
          title={`Editar ${partnerType === "WELLNESS" ? "Parceiro Wellness" : "Lojista Parceiro"}`}
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="SUPPLIER">Lojista parceiro</SelectItem>
